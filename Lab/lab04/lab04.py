...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    your_location = make_city('you', lat, lon)
    if distance(your_location, city2) > distance(your_location, city1):
...
def replace_elem(lst, index, elem):
    ...
    lst = lst[0:index] + [elem] + lst[index+1:]
    return lst
...
def put_piece(board, max_rows, column, player):
    ...
    rows = max_rows - 1
    while(rows>=0):
        if get_piece(board, rows, column) == '-':
            board = replace_elem(board, rows, replace_elem(board[rows], column, player))
            return rows, board
        else:
            rows -= 1
    return -1, board
